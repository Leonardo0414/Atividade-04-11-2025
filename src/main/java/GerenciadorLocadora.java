...
public class GerenciadorLocadora {
    ...
    public GerenciadorLocadora(DatabaseManager dbManager) {
    ...
    public void cadastrarCliente(Cliente cliente) {
        ...
        dbManager.getClienteRepo().insert(cliente);
    ...
    public void realizarAluguel() {
        ...
        dbManager.getLocacaoRepo().insert(novaLocacao);
        atualizarMidia(midiaSelecionada);
    ...
    public void devolverMidia() {
        ...
        atualizarMidia(midiaDevolvida);
        dbManager.getLocacaoRepo().update(locacaoAtiva);
    ...
    private void carregarDados() {
        dbManager.getClienteRepo().find().forEach(clientes::add);
        dbManager.getDvdRepo().find().forEach(acervo::add);
        dbManager.getFitaRepo().find().forEach(acervo::add);
        dbManager.getStreamingRepo().find().forEach(acervo::add);
        dbManager.getLocacaoRepo().find().forEach(historicoLocacoes::add);
        
        if (!clientes.isEmpty()) {
            System.out.println("Dados carregados do banco: " + clientes.size() + " clientes, " + 
                             acervo.size() + " mídias, " + historicoLocacoes.size() + " locações.");
        }
    }

    private void salvarMidia(Midia midia) {
        if (midia instanceof DVD) {
            dbManager.getDvdRepo().insert((DVD) midia);
        } else if (midia instanceof FitaVHS) {
            dbManager.getFitaRepo().insert((FitaVHS) midia);
        } else if (midia instanceof Streaming) {
            dbManager.getStreamingRepo().insert((Streaming) midia);
        }
    }

    private void atualizarMidia(Midia midia) {
        if (midia instanceof DVD) {
            dbManager.getDvdRepo().update((DVD) midia);
        } else if (midia instanceof FitaVHS) {
            dbManager.getFitaRepo().update((FitaVHS) midia);
        } else if (midia instanceof Streaming) {
            dbManager.getStreamingRepo().update((Streaming) midia);
        }
    }
}